...
function functionNameYouGive(code, query) {
  ...
  const stem = (word) => {
    ...
    if (word.endsWith('e') && word.length > 3) return word.slice(0, -1); // Added e removal
    return word;
  };
    ...
    if (score > 0) {
        ...
        if (name) {
          ...
          nameTokens.forEach(nt => {
            // Calculate IDF for the name token
            const df = docFrequencies[nt] || 1;
            const idf = Math.log(1 + (N - df + 0.5) / (df + 0.5));
            
            // Exact name match
            if (queryTokens.includes(nt)) score += 5 * idf; 
            // Substring name match
            else {
                queryTokens.forEach(qt => {
                    if (nt.includes(qt) || qt.includes(nt)) score += 3 * idf;
                });
            }
          });
        }
