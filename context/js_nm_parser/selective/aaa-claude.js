...
function findTopRelevantLineRangesClaude(code, query) {
  ...
  const semanticExpansions = {
    ...
    "health": ["health", "getsystemhealth", "system"],
    "uptime": ["uptime", "starttime", "start", "time"],
    "success": ["success", "successrate", "completed", "rate"],
    "queue": ["queue", "queued", "push", "shift"],
    "task": ["task", "tasks", "registertask", "register"],
    "config": ["config", "configuration", "configurationmanager", "loadfromenv", "env", "environment", "load"],
    "metrics": ["metrics", "metricsexporter", "appendfile", "filepath", "readhistory", "tojson", "exporter"],
    "disk": ["disk", "diskusage", "checkdiskusage", "size", "directory", "check", "usage"],
    "filesystem": ["filesystem", "traversal", "readdir", "stat", "recursive", "checkdiskusage", "directory", "files", "sync"],
    "traversal": ["traversal", "traverse", "readdir", "directory", "recursive", "walk", "files"],
    ...
    "cpu": ["cpu", "intensive", "computation", "complexmathalgorithm", "math", "algorithm", "complex", "compute"],
    "periodic": ["periodic", "interval", "setinterval", "report", "status"],
    "concurrency": ["concurrency", "concurrencylimit", "limit", "running", "max"],
    ...
    enter(path) {
      ...
      if (isFunction || isClass || (isTopLevel && isStatement) || isObjectProperty) {
        ...
        if (matches > 0) {
          ...
          if (nodeText.includes('readdir') && nodeText.includes('Directory') && queryTokens.some(t => t.includes('filesystem') || t.includes('traversal'))) score *= 4.0;
          ...
          // Q28: CPU intensive computation - boost complexMathAlgorithm (only by name, not content)
          if (name === 'complexmathalgorithm') score *= 3.0;
