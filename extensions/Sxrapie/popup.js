function updateStats() {
  chrome.runtime.sendMessage({ action: "getStats" }, (response) => {
    if (response) {
      ...
      document.getElementById('itemCount').innerText = `📁 ${response.count} profiles${sizeInfo}`;
      
      const urlList = document.getElementById('urlList');
      urlList.innerHTML = '';
      
      if (response.summaries && response.summaries.length > 0) {
        response.summaries.slice(-10).reverse().forEach(profile => {
          const div = document.createElement('div');
          const icon = profile.captureMode === 'auto' ? '🤖' : '👆';
          div.textContent = `${icon} ${profile.fullName || profile.profileId || 'Unknown'}`;
          div.title = profile.url;
          div.style.cursor = 'pointer';
          div.onclick = () => chrome.tabs.create({ url: profile.url });
...
document.getElementById('captureBtn').addEventListener('click', async () => {
  ...
  chrome.runtime.sendMessage({ action: "capture" }, (response) => {
    ...
    setTimeout(() => {
      btn.innerText = '👆 CAPTURE THIS PAGE';
...
document.getElementById('downloadBtn').addEventListener('click', () => {
  chrome.storage.local.get({ scrapedData: {} }, (result) => {
    ...
    downloadAnchorNode.setAttribute("download", `ghost_profiles_${Date.now()}.json`);
